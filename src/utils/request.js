...
const http = function (methods, url, oData, urlType, config) {
    ...
    if (Authorization) {
        switch (methods.toLowerCase()) {
            case 'get':
                return config ? service.get(url, { params: oData, ...config }) : service.get(url, { params: oData });
            case 'post':
                return service.post(url, oData, config);
            case 'put':
                return service.put(url, oData, config);
            case 'delete':
                return config ? service.delete(url, { params: oData, ...config }) : service.delete(url, { params: oData });
        }
    } else {
